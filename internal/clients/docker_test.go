...
func TestTrackProviderConfigUsage(t *testing.T) {
	...
	tests := []struct {
		...
		{
			name: "no provider config reference",
			setupClient: func() *fake.ClientBuilder {
				return fake.NewClientBuilder().WithScheme(scheme)
			},
			setupMG: func() *v1alpha1.Container {
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			...
			err := TrackProviderConfigUsage(context.Background(), kubeClient, mg)

			if (err != nil) != tt.wantError {
				t.Errorf("TrackProviderConfigUsage() error = %v, wantErr %v", err, tt.wantError)
			}
		})
	}
}
...
func TestNotFoundError(t *testing.T) {
	tests := []struct {
	...
	}{
		{
			...
			resourceID:   "test123",
			want:         "container test123 not found",
		},
		{
			name:         "image not found",
			resourceType: "image",
			resourceID:   "nginx:latest",
			want:         "image nginx:latest not found",
		},
	}
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			...
			if err.Error() != tt.want {
				t.Errorf("NotFoundError.Error() = %v, want %v", err.Error(), tt.want)
			}
		})
	}
}

func TestExtractCredentialsErrorCases(t *testing.T) {
	scheme := runtime.NewScheme()
	_ = v1beta1.SchemeBuilder.AddToScheme(scheme)
	_ = corev1.AddToScheme(scheme)
	...
	tests := []struct {
		...
		},
		{
			...
			wantError: true,
			errorMsg:  "credentials source invalid-source is not currently supported",
		},
	}
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			...
			_, err := ExtractCredentials(context.Background(), kubeClient, tt.providerConfig)
			...
			if tt.wantError {
				...
				if err != nil {
					t.Errorf("ExtractCredentials() expected no error but got: %v", err)
				}
			}
		})
	}
...
func TestCreateDockerClientEdgeCases(t *testing.T) {
	tests := []struct {
		name           string
		providerConfig *v1beta1.ProviderConfig
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			_, err := createDockerClient(tt.providerConfig, tt.credentials)

			if tt.wantError {
				if err == nil {
					t.Errorf("createDockerClient() expected error but got none")
					return
				}
				if tt.errorMsg != "" && !contains(err.Error(), tt.errorMsg) {
					t.Errorf("createDockerClient() error = %v, want error containing %v", err, tt.errorMsg)
				}
			} else {
				if err != nil {
					t.Errorf("createDockerClient() expected no error but got: %v", err)
				}
			}
		})
	}
